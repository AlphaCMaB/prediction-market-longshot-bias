...
1. Design Phase 10F's outcome-blind one-hour price-acquisition planner over the
   167,954 verified families, including exact request and compressed-storage
   preflight, market-existence attrition, and the 15-minute staleness rule.
2. Resolve the generated-namespace constraint before any production network
   run. Only 156,396,353 bytes remain, so a bounded smoke and measured storage
   model must precede any archival/pruning approval request.
3. Construct and freeze the horizon-price pilot sample without outcomes. Merge
   quarantined outcomes only after all features and inclusion rules are frozen.
