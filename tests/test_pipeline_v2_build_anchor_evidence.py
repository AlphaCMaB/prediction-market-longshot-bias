...
MARKET_FIELDS = (
    "ticker", "family_id", "family_id_source", "event_ticker", "title",
    "subtitle", "rules_primary", "rules_secondary", "market_open_time",
    "occurrence_datetime", "diagnostic_settlement_ts", "close_time", "expiration_time",
)
EVENT_FIELDS = (
    ...
    markets = [
        {
            ...
            "event_ticker": "E1", "title": "Market 1", "subtitle": "", "rules_primary": "Rules",
            "rules_secondary": "", "market_open_time": "2025-06-01T00:00:00Z",
            "occurrence_datetime": "2025-08-01T12:00:00Z",
            "diagnostic_settlement_ts": f"2026-01-0{1 if retrospective == 'one' else 2}T00:00:00Z",
            "close_time": retrospective, "expiration_time": retrospective,
        },
        {
            "ticker": "M2", "family_id": "F1", "family_id_source": "source_b",
            "event_ticker": "E2", "title": "Market 2", "subtitle": "", "rules_primary": "Rules",
            "rules_secondary": "", "market_open_time": "2025-06-02T00:00:00Z",
            "occurrence_datetime": "", "diagnostic_settlement_ts": retrospective,
            "close_time": retrospective, "expiration_time": retrospective,
        },
    ]
    events = [
        {
            ...
            "sub_title": "", "category": "Sports", "strike_date": "2025-08-01",
            "strike_period": "", "mutually_exclusive": "", "settlement_sources_json": "[]",
            "product_metadata_json": "{}", "last_updated_ts": retrospective,
        },
        {
            "event_ticker": "E2", "series_ticker": "S2", "title": "Event 2",
            "sub_title": "", "category": "Economics", "strike_date": "",
            "strike_period": "", "mutually_exclusive": "", "settlement_sources_json": "[]",
            "product_metadata_json": "{}", "last_updated_ts": retrospective,
        },
    ]
    milestones = [{
        "event_ticker": "E1", "milestone_id": "MS1", "milestone_category": "event",
        "milestone_type": "start", "milestone_title": "Start",
        "milestone_start_date": "2025-08-01T12:00:00Z",
        "milestone_end_date": "2025-08-01T13:00:00Z", "milestone_source_id": "",
        "milestone_source_ids_json": "[]", "milestone_details_json": "{}",
        "milestone_last_updated_ts": retrospective, "association_type": "both",
    }]
    if reverse:
        markets.reverse()
        events.reverse()
...
def test_outputs_schemas_hashes_and_composite_counts(tmp_path):
    ...
    assert tuple(rows(output / "anchor_family_review.csv")[0]) == ANCHOR_FAMILY_REVIEW_FIELDS
    assert tuple(rows(output / "anchor_verification_decisions_template.csv")[0]) == DECISION_TEMPLATE_FIELDS
...
def test_template_is_accepted_and_leaves_all_families_unverified(tmp_path):
    ...
    applied_fields = tuple(dict.fromkeys((*MARKET_FIELDS, *(
        "verification_status", "verified_anchor_time", "verified_anchor_source",
        "timing_structure_reviewed", "evidence_reference", "review_note",
    ))))
    ...
    assert all(row["validation_status"] == "invalid_or_unverified" for row in rows(anchors_path))
    ...
    verified_identities = {
        ...
        for row in verified if row["verification_status"] == "verified_manual"
    }
    ...
    anchored = [
        row for row in rows(verified_anchors_path)
        if row["validation_status"] == "verified"
    ]
    assert len({
        (row["family_id"], row["family_id_source"]) for row in anchored
    }) == 1
...
def test_input_order_and_retrospective_updates_do_not_change_outputs(tmp_path):
    ...
    for name in (
        "anchor_evidence.csv", "anchor_family_review.csv",
        "anchor_verification_decisions_template.csv", "anchor_evidence_report.json",
    ):
...
def test_no_automatic_verification_language_or_values(tmp_path):
    ...
    output = tmp_path / "out"
    run(*inputs, output, config_path=CONFIG)
    combined = b"".join((output / name).read_bytes() for name in (
        "anchor_evidence.csv", "anchor_family_review.csv",
    ...
    assert {row["verification_status"] for row in rows(
        output / "anchor_verification_decisions_template.csv"
    )} == {"needs_review"}
